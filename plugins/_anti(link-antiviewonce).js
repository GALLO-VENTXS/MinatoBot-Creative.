...
handler.before = async function (m, { isAdmin, isBotAdmin }) {
    ...
    if (chat.antiLink && isGroupLink && !isAdmin && !m.isBaileys && m.isGroup) {
        ...
        if (!isBotAdmin) m.reply(` *「 ANTILINK 」* ${isAdmin ? "No soy admin, no puedo eliminarte :'v" : `\n\nSe detectó un enlace de grupo y ${global.namebot} no es admin, por lo que no puede dar kick.`}`)
        if (isBotAdmin) {
            m.reply(` *「 ANTILINK 」* \n\nSe detectó enlace de grupo, ¡¡adiós, serás expulsado!!`.trim())
